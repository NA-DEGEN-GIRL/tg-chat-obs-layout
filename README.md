...
`TELEGRAM_USER_SEND_ENABLED=1`을 켜면 로컬 `/api/send`가 열립니다. `TELEGRAM_USER_SEND_PANEL=1`도 켜면 9292 오버레이에 작은 입력창이 생기고, 거기서 보낸 메시지가 사용자 계정으로 메인 채팅과 현재 `/here_on` 대상에 전송됩니다.
...
입력창에서 보낸 텍스트는 Telethon 사용자 세션으로 전송됩니다. 전송 대상은 항상 메인 `CHAT_ID`이고, `/here_on`이 활성화되어 있으면 해당 thread/comment chat에도 같이 보냅니다.
