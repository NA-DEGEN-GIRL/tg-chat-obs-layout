#!/usr/bin/env python3
"""React Suspense/lazy smoke test through browser_lab.html.

This covers a common production pattern not exercised by simple CSR fixtures:
the initial page renders a Suspense fallback, the real form appears later from a
lazy component, and Browser Lab must still forward events into React's delegated
event system after that delayed render.
"""
...
STATIC_PORT = 9590
FIXTURE_PORT = 9591
PROFILE_DIR = Path("/tmp/tg-browser-lab-profile-react-suspense-smoke")
RUNTIME_DIR = Path("/tmp/tg-browser-lab-runtime-react-suspense-smoke")
FRAME_PATH = RUNTIME_DIR / "react_suspense_smoke_final.jpg"
...
<head>
  <meta charset="utf-8">
  <title>Browser Lab React Suspense Smoke</title>
  <style>
    body { font-family: sans-serif; padding: 18px; }
    label { display: block; margin: 8px 0; }
    input, select, button { font-size: 20px; padding: 8px; margin: 6px; }
    input[type="checkbox"] { width: auto; }
    #fallback { font-size: 22px; padding: 14px; border: 2px dashed #777; width: 320px; }
    #suspenseResult { margin-top: 16px; font-size: 22px; font-weight: 700; white-space: pre-wrap; }
  </style>
</head>
<body>
  ...
  <script>
    ...
    function LazyForm() {
      ...
      window.__browserLabSuspenseSmokeState = state;
      ...
      return e('form', {
        ...
        e('h1', null, 'Browser Lab React Suspense Smoke'),
        e('label', { htmlFor: 'suspenseName' }, 'Suspense name'),
        e('input', {
          id: 'suspenseName',
          value: name,
          onChange: updateName,
        ...
        e('button', { id: 'suspenseSubmit', type: 'submit' }, 'Submit'),
        e('div', { id: 'suspenseResult' },
          ok ? 'ok:suspense:' + name + ':' + choice + ':' + clicks + ':' + (submitted ? 'submitted' : 'open')
             : ready ? 'state:' + JSON.stringify(state) : 'state:pending'
        )
      );
    }

    const LazyLoadedForm = React.lazy(() => new Promise((resolve) => {
      window.setTimeout(() => resolve({ default: LazyForm }), 800);
    }));

    function App() {
      return e(React.Suspense, { fallback: e('div', { id: 'fallback' }, 'Loading lazy React form...') },
        e(LazyLoadedForm)
      );
    }
...
async def suspense_state(controller: Any) -> Dict[str, Any]:
    if not controller.page:
        return {}
    state = await controller.page.evaluate("window.__browserLabSuspenseSmokeState || null")
    return state if isinstance(state, dict) else {}


async def wait_for_suspense_state(
    controller: Any,
    ...
    last: Dict[str, Any] = {}
    while time.monotonic() - started < timeout:
        last = await suspense_state(controller)
...
async def run_smoke() -> None:
    ...
    try:
        ...
        async with async_playwright() as playwright:
            ...
            await wait_for(remote_fixture_loaded, "Remote React Suspense fixture navigation", timeout=35)
            if not controller.page:
                raise RuntimeError("Remote browser page is not available")
            await controller.page.locator("#fallback").wait_for(state="visible", timeout=20_000)
            await controller.page.locator("#suspenseName").wait_for(state="visible", timeout=35_000)
            await wait_for_suspense_state(controller, {"ready": True}, "React Suspense lazy form ready", timeout=20)
            await host_page.locator("#viewportCanvas").scroll_into_view_if_needed()

            await click_remote_selector(host_page, controller, "#suspenseName")
            await host_page.keyboard.type("suspense")
            ...
            await wait_for_suspense_state(
                controller,
                {"name": "suspense\uD55C\uAE00", "compositionOk": True},
                "React Suspense controlled input and composition",
            )

            await click_remote_selector(host_page, controller, "#suspenseAgree")
            await wait_for_suspense_state(controller, {"agree": True}, "React Suspense checkbox")
            await click_remote_selector(host_page, controller, "#suspenseChoice")
            await host_page.keyboard.press("ArrowDown")
            await host_page.keyboard.press("Enter")
            await wait_for_suspense_state(controller, {"choice": "b"}, "React Suspense select")
            await click_remote_selector(host_page, controller, "#suspenseButton")
            await wait_for_suspense_state(controller, {"clicks": 1}, "React Suspense button click")
            await click_remote_selector(host_page, controller, "#suspenseSubmit")
            await wait_for_suspense_state(controller, {"ok": True}, "React Suspense form submit")

            result = await controller.page.locator("#suspenseResult").inner_text()
            expected = "ok:suspense:suspense\uD55C\uAE00:b:1:submitted"
            if result != expected:
                raise AssertionError(f"React Suspense smoke failed: expected {expected!r}, got {result!r}")
